...
ReactNative::ReactInstanceManager ReactInstanceManagerBuilder::Build() {
  ...
  if (!m_useDeveloperSupport && m_jsBundleFile.empty()) {
    throw hresult_invalid_operation(L"JavaScriptBundleFile must be provided when UseDeveloperSupport is false.");
  }

  if (m_jsBundleFile.empty() && m_jsMainModuleName.empty()) {
    throw hresult_invalid_operation(L"Either MainModulePath or the JavaScriptBundleFile must be provided.");
  }
