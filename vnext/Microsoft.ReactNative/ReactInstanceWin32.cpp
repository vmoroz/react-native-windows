...
#include "ReactImpl_Win.h" // For telemetry types
...
ETWSystraceHandler s_etwSystraceHandler;
Mso::ThreadSafeLazyInitGuard s_initOnceInitTracingAndTelemetry;
...
{
	LoadedCallbackGuard(ReactInstanceWin32& reactInstance) noexcept
...
private:
	Mso::TCntPtr<ReactInstanceWin32> m_reactInstance;
};
...
ReactInstanceWin32::ReactInstanceWin32(
	...
	, m_rekaContextProxy { &rekaContextProxy }
	, m_whenLoaded { std::move(whenLoaded) }
{
	m_rekaContextProxy->ResetState();
}
...
void ReactInstanceWin32::Initialize() noexcept
{
	...
	devSettings->bytecodeFileName = GetBytecodeFileName();
...
void ReactInstanceWin32::LoadJSBundles() noexcept
{
				...
				try
				{
					...
					if (PathFileExistsA(userJSBundleFilePath.c_str()))
					{
						if (options.JSBundles.size() == 0
							|| _stricmp(userJSBundleFilePath.c_str(), options.JSBundles[options.JSBundles.size() - 1]->Info().FileName.c_str()) != 0)
						{
							// Do an early exit if instance is already destroyed.
							if (strongThis->m_isDestroyed)
							{
							...
							//AssertSz(false, ("JS Bundle is being loaded through Legacy ReactOptions.Identity Support. Please switch to ReactOptions.JSBundles. JS Bundle: " + userJSBundleFilePath).c_str());
	...
	else
	{
		...
		if (m_options.JSBundles.size() > 0
			&& _stricmp(userJSBundleFilePath.c_str(), m_options.JSBundles[m_options.JSBundles.size() - 1]->Info().FileName.c_str()) != 0)
		{
			if (PathFileExistsA(userJSBundleFilePath.c_str()))
			{
				// JS Bundle does not exist in ReactOptions.JSBundle, but exists through Identity. Debugging and Live Reload are supported.
				AssertSzTag(false, ("Web Debugging and Live Reload JS Bundle is being loaded through Legacy ReactOptions.Identity Support. Please switch to ReactOptions.JSBundles. JS Bundle: " + userJSBundleFilePath).c_str(), 0x0285e289 /* tag_c74kj */);
				instance->setGlobalVariable("__platformBundles", std::make_unique<facebook::react::JSBigStdString>(folly::toJson(platformBundleArray)));
				instanceWrapper->loadBundle(Mso::Copy(m_options.Identity));
			}
			else
			{
				// JS Bundle exists in ReactOptions.JSBundle, but not in Identity. Debugging and Live Reload are not supported.
				//TODO: [vmorozov][vmorozov:3531328] Address this scenario
				AssertSzTag(false, "Web Debugging and Live Reload is not yet supported for ReactOptions.JSBundles. Tracked by: 2960589:Support Web Debugging and Live Reload for ReactOptions.JSBundles", 0x0285e28a /* tag_c74kk */);
			}
		}
		else
		{
...
void ReactInstanceWin32::OnReactInstanceLoaded(const Mso::ErrorCode& errorCode) noexcept
{
	if (!m_isLoaded)
	{
		Mso::Async::InvokeElsePost(&Queue(),
			[weakThis = Mso::MakeWeakPtr(this), errorCode]() noexcept {
			if (auto strongThis = weakThis.GetStrongPtr())
			{
				if (!strongThis->m_isLoaded)
				{
					strongThis->m_isLoaded = true;
					bool isInError = static_cast<bool>(errorCode);
					...
					{
						strongThis->m_rekaContextProxy->SetError(Mso::Copy(errorCode));
						if (auto rekaInstance = strongThis->m_rekaInstance.Exchange(nullptr))
						{
...
Mso::Future<void> ReactInstanceWin32::Destroy() noexcept
{
	...
	NotifyRekaDestroyed();
	if (auto rekaInstance = m_rekaInstance.Exchange(nullptr))
	{
		rekaInstance->Destroy();
	}
...
std::shared_ptr<facebook::react::MessageQueueThread> ReactInstanceWin32::DefaultNativeMessageQueueThread() const noexcept
{
...
void ReactInstanceWin32::InitTracingAndTelemetry() noexcept
{
	s_initOnceInitTracingAndTelemetry.InitOnce([]() noexcept->bool
	{
		EventRegisterOfficeReactNativeHost();
		auto spClient = std::make_shared<Mso::Telemetry::SimpleTelemetryClient<Mso::Telemetry::guidProviderOfficeReactNativeHost>>();
		Mso::Telemetry::RegisterEtwManifests(spClient, spClient->GetGUIDs());
		Mso::Telemetry::RefreshDataCollectorSources();
		facebook::react::InitializeTracing(&s_etwSystraceHandler);
		return true;
	});
}
...
void ReactInstanceWin32::InitJSMessageThread() noexcept
{
	...
	if (jsDispatchQueue)
	{
		VerifyElseCrashSzTag(Mso::Async::IsSequentialQueue(*jsDispatchQueue), "JS Queue must be sequential", 0x0285e28b /* tag_c74kl */);
	}
	else
	{
	...
	m_jsMessageThread.Exchange(std::make_shared<FastMessageQueueThread>(
		*jsDispatchQueue,
		Mso::MakeWeakMemberFunctor(this, &ReactInstanceWin32::OnError),
...
void ReactInstanceWin32::InitUIManager() noexcept
{
	...
	// If we couldn't find a NetUI UIManager, we are initializing an instance used for hosting a UIless scenario, such as UDF
...
CxxModuleProviders ReactInstanceWin32::GetCxxModuleProviders() noexcept
{
	...
	for (const auto& cxxModuleName : m_options.CxxModuleNames)
	{
		...
		VerifyElseCrashSzTag(registeredCxxModuleProvider, "Could not find the registeredCxxModuleProvider for the specified cxx module name", 0x0285e28d /* tag_c74kn */);
		...
		for (const auto& cxxModuleDescription : cxxModuleDescriptions)
		{
			...
			facebook::xplat::module::CxxModule::Provider cxxModuleProvider([factory = std::get<1>(cxxModuleDescription)]() {
				return factory();
			});
			...
			cxxModuleProviders.emplace_back(std::move(moduleName), std::move(cxxModuleProvider), std::move(moduleMessageThread));
		}
	}

	RegisterRekaCxxModuleProvider(cxxModuleProviders);

	return cxxModuleProviders;
}

void ReactInstanceWin32::RegisterRekaCxxModuleProvider(CxxModuleProviders& cxxModuleProviders) noexcept
{
	Mso::JSHost::RekaBridgeOptions rekaOptions {};

	if (m_options.RekaProviderFactory)
	{
		// Custom Provider Factory
		rekaOptions.RekaServiceProviderFactory = m_options.RekaProviderFactory;
	}
	else
	{
		// Default Provider Factory
		rekaOptions.RekaServiceProviderFactory =
			[rekaServiceProviderName = m_options.DataServiceProviderName](Mso::JSHost::IRekaContext& context) mutable noexcept
		{
			return Mso::JSHost::GetOrMakeRekaServiceProvider(rekaServiceProviderName.c_str(), context);
		};
	}

	rekaOptions.OnInitialized = [weakThis = Mso::MakeWeakPtr(this)](Mso::JSHost::IRekaContext& context) noexcept
	{
		if (auto strongThis = weakThis.GetStrongPtr())
		{
	...
	rekaOptions.OnDestroyed = [weakThis = Mso::MakeWeakPtr(this)](Mso::JSHost::IRekaContext& /*context*/) noexcept
	{
		if (auto strongThis = weakThis.GetStrongPtr())
		{
	...
	m_rekaInstance.Exchange(Mso::React::MakeRekaInstance(std::move(rekaOptions), this));

	facebook::xplat::module::CxxModule::Provider rekaCxxModuleProvider(
		[rekaInstance = m_rekaInstance.Load()]() mutable noexcept {
		return std::make_unique<RekaNativeModule>(std::move(rekaInstance));
	});

	cxxModuleProviders.emplace_back(RekaNativeModule::ModuleName(), std::move(rekaCxxModuleProvider), m_jsMessageThread.Load());
}

void ReactInstanceWin32::NotifyRekaInitialized() noexcept
{
...
void ReactInstanceWin32::NotifyRekaDestroyed() noexcept
{
...
std::shared_ptr<facebook::react::MemoryTracker> ReactInstanceWin32::GetMemoryTracker() noexcept
{
	if (m_options.MemoryTracker)
	{
		auto getMessageQueueThread = [this](Mso::Async::IDispatchQueue * memoryTrackerDQ) noexcept->std::shared_ptr<facebook::react::MessageQueueThread>
		{
			if (memoryTrackerDQ != nullptr && memoryTrackerDQ != &Queue())
			{
				return std::make_shared<FastMessageQueueThread>(*memoryTrackerDQ, MakeWeakMemberFunctor(this, &ReactInstanceWin32::OnError));
			}
			else
			{
				return m_nativeMessageThread.LoadWithLock();
		...
		return static_cast<Mso::React::MemoryTrackerImpl*>(m_options.MemoryTracker.Get())->GetReactMemoryTracker(getMessageQueueThread);
	}
...
void ReactInstanceWin32::CallJsFunction(std::string&& moduleName, std::string&& method, folly::dynamic&& params) noexcept
{
