...
LIBLET_PUBLICAPI ReactOptions& ReactOptions::AddRegisteredJSBundle(::string_view jsBundleId) noexcept
{
...
LIBLET_PUBLICAPI ReactOptions& ReactOptions::AddFileJSBundle(::string_view jsBundleId, ::string_view fileName) noexcept
{
...
/*static*/ Mso::TCntPtr<Mso::Async::IDispatchQueue> ReactHost::GetNativeQueue(const ReactOptions& options) noexcept
{
	...
	else
	{
		...
		{
			nativeDispatchQueue = Mso::Async::CreateBackgroundSequentialDispatchQueue();
		}
	}
...
Mso::TCntPtr<IReactViewHost> ReactHost::MakeViewHost(ReactViewOptions&& options) noexcept
{
	return Mso::Make<ReactViewHost, IReactViewHost>(*this, std::move(options));
}

Mso::Future<std::vector<Mso::TCntPtr<IReactViewHost>>> ReactHost::GetViewHostList() noexcept
{
...
{
	return m_reactHost->PostInQueue([this, viewInstance = Mso::MakeTCntPtr(&viewInstance)]() noexcept
	{
