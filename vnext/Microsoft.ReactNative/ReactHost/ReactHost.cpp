...
LIBLET_PUBLICAPI ReactOptions &ReactOptions::AddRegisteredJSBundle(
    ::string_view jsBundleId) noexcept {
...
LIBLET_PUBLICAPI ReactOptions &ReactOptions::AddFileJSBundle(
    ::string_view jsBundleId,
    ::string_view fileName) noexcept {
  this->JSBundles.push_back(
      MakeFileJSBundle(jsBundleId.data(), fileName.data()));
  return *this;
}
...
Mso::Future<void> AsyncActionQueue::PostActions(
...
void AsyncActionQueue::InvokeAction(Entry &&entry) noexcept {
  ...
  actionResult.Then(
      m_executor,
      [ entry = std::move(entry), spThis = Mso::MakeTCntPtr(this) ](
          Mso::Maybe<void> && result) mutable noexcept {
        spThis->CompleteAction(std::move(entry), std::move(result));
      });
}

void AsyncActionQueue::CompleteAction(
    Entry &&entry,
    Mso::Maybe<void> &&result) noexcept {
  // Complete the action
...
Mso::Async::IDispatchQueue &AsyncActionQueue::Queue() noexcept {
  return *m_queue;
}
...
void ReactHost::Initialize() noexcept {
  auto whenLoaded = m_actionQueue.Load()->PostAction(
      MakeLoadInstanceAction(Mso::Copy(m_options.Load())));

  // We only use the m_onInstanceLoaded for the initial notification and then
  // set it to null.
  Mso::SetPromiseValue(
      m_onInstanceLoaded.Exchange(nullptr), std::move(whenLoaded));
}
...
void ReactHost::Finalize() noexcept {
  ...
  // Since each AsyncAction has a strong ref count to ReactHost, the
  // AsyncActionQueue must be empty. Thus, we only need to call UnloadInQueue to
  // unload ReactInstance if the ReactHost is not closed yet.
  if (Mso::Promise<void> notifyWhenClosed =
          m_notifyWhenClosed.Exchange(nullptr)) {
    UnloadInQueue(0).Then<Mso::Executors::Inline>(
        [notifyWhenClosed = std::move(notifyWhenClosed)]() noexcept {
...
void ReactHost::Close() noexcept {
  InvokeInQueue([this]() noexcept {
    // Put the ReactHost to the closed state, unload ReactInstance, and notify
    // the closing Promise.
    auto whenClosed =
        m_actionQueue.Load()->PostAction(MakeUnloadInstanceAction());

    // After we set the m_notifyWhenClosed to null, the ReactHost is considered
    // to be closed.
    Mso::SetPromiseValue(
        m_notifyWhenClosed.Exchange(nullptr), std::move(whenClosed));
  });
}
...
Mso::TCntPtr<AsyncActionQueue> ReactHost::ActionQueue() const noexcept {
...
/*static*/ Mso::TCntPtr<Mso::Async::IDispatchQueue> ReactHost::GetNativeQueue(
    const ReactOptions &options) noexcept {
  auto nativeDispatchQueue =
      options.Properties.Get(NativeDispatchQueueProperty);
  if (nativeDispatchQueue) {
    VerifyElseCrashSzTag(
        Mso::Async::IsSequentialQueue(*nativeDispatchQueue),
        "NativeQueue must be sequential",
        0x0281e3d6 /* tag_c64pw */);
  } else {
    // Try to use current queue
    nativeDispatchQueue = &Mso::Async::CurrentQueue();
    if (!Mso::Async::IsSequentialQueue(*nativeDispatchQueue)) {
      nativeDispatchQueue =
          Mso::Async::CreateBackgroundSequentialDispatchQueue();
    }
  }
...
Mso::TCntPtr<IReactInstance> ReactHost::Instance() const noexcept {
...
Mso::Async::IDispatchQueue &ReactHost::NativeQueue() const noexcept {
  return Queue();
}
...
Mso::Future<void> ReactHost::ReloadInstanceWithOptions(
...
Mso::TCntPtr<IReactViewHost> ReactHost::MakeViewHost(
...
ReactHost::GetViewHostList() noexcept {
  Mso::Promise<std::vector<Mso::TCntPtr<IReactViewHost>>> result;
  InvokeInQueue([ this, result ]() noexcept {
    std::vector<Mso::TCntPtr<IReactViewHost>> viewHosts;
...
Mso::Future<void> ReactHost::LoadInQueue(ReactOptions &&options) noexcept {
  ...
  // Make sure that we set new options even if we do not load due to the pending
  // unload.
  ...
  m_reactInstance.Exchange(MakeReactInstance(
      *this, std::move(options), *m_rekaContextProxy, Mso::Copy(onLoaded)));

  // After the instance is loaded, we load the view instances.
  // It is safe to capture 'this' because the Load action keeps a strong
  // reference to ReactHost.
  return onLoaded.AsFuture().Then(
      m_executor, [this](Mso::Maybe<void> && /*value*/) noexcept {
        ...
        ForEachViewHost([&loadCompletionList](auto &viewHost) noexcept {
          loadCompletionList.push_back(
              viewHost.LoadViewInstanceInQueue(viewHost.Options()));
        });
...
Mso::Future<void> ReactHost::UnloadInQueue(size_t unloadActionId) noexcept {
  // If the pending unload action Id does not match, then we have newer unload
  // action, and thus we should cancel this one.
  ...
  // It is safe to capture 'this' because the Unload action keeps a strong
  // reference to ReactHost.
  return Mso::WhenAllCompleted(unloadCompletionList)
      .Then(m_executor, [this](Mso::Maybe<void> && /*value*/) noexcept {
...
void ReactHost::ForEachViewHost(
...
void ReactHost::AttachViewHost(ReactViewHost &viewHost) noexcept {
  ...
  VerifyElseCrashSzTag(
      it == viewHosts.end(),
      "The view host is already attached",
      0x0281e3d8 /* tag_c64py */);
...
void ReactHost::DetachViewHost(ReactViewHost &viewHost) noexcept {
  ...
  VerifyElseCrashSzTag(
      it != viewHosts.end(),
      "The view host is not attached",
      0x0281e3d9 /* tag_c64pz */);
  viewHosts.erase(it);
}
...
Mso::Future<void> ReactViewHost::ReloadViewInstance() noexcept {
  return m_reactHost->PostInQueue([this]() noexcept {
    return m_actionQueue.Load()->PostActions(
        {MakeUnloadViewInstanceAction(), MakeLoadViewInstanceAction()});
  });
}

Mso::Future<void> ReactViewHost::ReloadViewInstanceWithOptions(
    ReactViewOptions &&options) noexcept {
  return m_reactHost->PostInQueue(
      [ this, options = std::move(options) ]() mutable noexcept {
        return m_actionQueue.Load()->PostActions(
            {MakeUnloadViewInstanceAction(),
...
Mso::Future<void> ReactViewHost::AttachViewInstance(
    IReactViewInstance &viewInstance) noexcept {
  return m_reactHost->PostInQueue(
      [ this, viewInstance = Mso::MakeTCntPtr(&viewInstance) ]() noexcept {
        auto previousViewInstance =
            m_viewInstance.Exchange(Mso::Copy(viewInstance));
        VerifyElseCrashSzTag(
            !previousViewInstance,
            "ViewInstance must not be previously attached.",
            0x028508d6 /* tag_c7q9w */);
        m_reactHost->AttachViewHost(*this);

        // Schedule the viewInstance load in the action queue since there can be
        // other load actions in the queue that need to be consolidated.
...
Mso::Future<void> ReactViewHost::DetachViewInstance() noexcept {
  return m_reactHost->PostInQueue([this]() noexcept {
    ...
    VerifyElseCrashSzTag(
        viewInstance,
        "ViewInstance is not attached.",
        0x0281e3db /* tag_c64p1 */);
    m_reactHost->DetachViewHost(*this);

    // We unload the viewInstance here as soon as possible without using the
    // action queue, otherwise we would not have the viewInstance to call the
    // Unload() later.
...
AsyncAction ReactViewHost::MakeLoadViewInstanceAction(
    ReactViewOptions &&options) noexcept {
  return [
    spThis = Mso::MakeTCntPtr(this),
    options = std::move(options)
...
Mso::Future<void> ReactViewHost::LoadViewInstanceInQueue() noexcept {
  ...
  // If there is a pending unload action, then we cancel loading
  // ReactViewInstance.
  ...
  if (auto viewInstance = m_viewInstance.Load().Get()) {
    ...
    return viewInstance->Reload(
        m_reactHost->Instance(), Mso::Copy(m_options.Load()));
  }

  return Mso::MakeCanceledFuture();
}

Mso::Future<void> ReactViewHost::LoadViewInstanceInQueue(
    ReactViewOptions &&options) noexcept {
  // Make sure that we set new options even if we do not load due to the pending
  // unload.
  m_options.Exchange(std::move(options));
...
Mso::Future<void> ReactViewHost::UnloadViewInstanceInQueue(
    size_t unloadActionId) noexcept {
  // If the pending unload action Id does not match, then we have newer unload
  // action, and thus we should cancel this one.
...
/*static*/ Mso::TCntPtr<ReactHostRegistry> ReactHostRegistry::s_registry;
...
/*static*/ void ReactHostRegistry::OnLibletInit() noexcept {
  ...
  VerifyElseCrashSzTag(
      !s_registry,
      "No ReactHostRegistry must exist yet.",
      0x0281e3dc /* tag_c64p2 */);
  s_registry = Mso::Make<ReactHostRegistry>();
}

/*static*/ void ReactHostRegistry::OnLibletUninit() noexcept {
  // Wait for all pending ReactHost futures indicating closing state and remove
  // the global ReactHostRegistry.
...
/*static*/ Mso::Promise<void> ReactHostRegistry::Register(
    ReactHost &reactHost) noexcept {
  std::lock_guard<std::mutex> lock{s_mutex};
  ...
  VerifyElseCrashSzTag(
      it == entries.end(),
      "The ReactHost is already registered",
      0x0281e3dd /* tag_c64p3 */);

  Mso::Promise<void> notifyWhenClosed;
  Mso::Future<void> whenReactHostClosed =
      notifyWhenClosed.AsFuture().Then<Mso::Executors::Inline>(
          [ registry{s_registry},
            reactHostKey ](Mso::Maybe<void> && result) noexcept {
            registry->Unregister(reactHostKey);
            return std::move(result);
          });
  entries.emplace(
      reactHostKey, Entry{&reactHost, std::move(whenReactHostClosed)});
...
void ReactHostRegistry::Unregister(uintptr_t reactHostKey) noexcept {
  ...
  VerifyElseCrashSzTag(
      it != m_entries.end(),
      "The ReactHost is not registered",
      0x0281e3de /* tag_c64p4 */);
  m_entries.erase(it);
}
...
LIBLET_PUBLICAPI OnErrorCallback GetDefaultOnErrorHandler() noexcept {
  return [](const Mso::ErrorCode &errorCode) noexcept {
    ...
    if (const std::exception_ptr *exc =
            errorCode.HandleAs(Mso::ExceptionErrorProvider())) {
      try {
        std::rethrow_exception(*exc);
      } catch (const std::exception &ex) {
        UNREFERENCED_PARAMETER(ex);
        AssertSzTag(false, ex.what(), 0x0281e3df /* tag_c64p5 */);
      }
    }

    ShipAssertTag(false, 0x0281e3e0 /* tag_c64p6 */);
  };
}
