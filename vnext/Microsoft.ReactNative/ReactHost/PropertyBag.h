...
//! Properties with null isolation key are considered global inside of the
//! PropertyBag instance. Isolation key allows to group properties or even
//! hide/isolate them between different components. Property name should be
//! unique for the same isolation key. Different isolation keys may have
//! properties with the same name.
//!
//! The PropertyBag class is a smart pointer to the IPropertyBag ref-counted
//! interface. The IPropertyBag defines three methods: Get, GetOrCreate, and
//! Replace:
...
//!                 Since property value creation is called outside of lock, the
//!                 valueMaker could be called multiple times, but only first
//!                 result is used. GetOrCreate may return null if valueMaker
//!                 returns null.
//! - Replace - replaces current property value with a new one and returns the
//! old value.
//!             Replacing property value with null, removes the property.
//!
//! On top of the untyped IPropertyBag interface, the PropertyBag offers a set
//! of typed property accessors: Get, GetOrCreate, Remove, and Set. There can be
//! two kinds of property value types:
//! - IUnknown inheriting interfaces. Each such type must have an associated
//! GUID and be QI-able from the IUnknown.
//!   Returning values are wrapped up into Mso::TCntPtr.
//! - Other types. For other types we use an internal wrapper and we require to
//! associate GUID with each such type.
//!   Macro MSO_PROPERTY_TYPE_GUID can be used to associate property types with
//!   GUID. There are some predefined association in this header file. Values of
//!   other types are wrapped up into Mso::optional.
//!
//! To simplify access to properties we offer NamedProperty and
//! IsolatedNamedProperty types that help to wrap up property type, name, and
//! isolation key into one constant.
//!
//! For example, we can define a property to access UI dispatch queue:
//!
//!     const Mso::JSHost::NamedProperty<Mso::Async::IDispatchQueue>
//!     UIQueueProperty{"UIQueue"};
//!
...
//!
//!     Mso::JSHost::RekaServiceWithQueueRegistration<TestService>
//!     vTestServce{UIQueueProperty};
//!

#include <compileradapters/cppmacros.h>
#include <core/FunctorRef.h>
...
//! To provide strongly typed access to non-IUnknown derived types we are
//! associating each type a with GUID. It is done by declaring a
//! GetPropertyTypeGuid function without implementation that receives a pointer
//! of target type and returns type with a GUID. The MSO_PROPERTY_TYPE_GUID can
//! be used do the associating with simple types. For template specializations
//! with a list of type parameters, you should write the same two lines as in
//! the MSO_PROPERTY_TYPE_GUID macro directly.
...
//! While it is possible to write PropertyTypeGuid<TProperty>::Value
//! specialization for the target type, the recommended way is to associate GUID
//! with a property type using MSO_PROPERTY_TYPE_GUID or declaring
...
//! Wraps up property values non-inherited from IUnknown into an IUnknown based
//! interface.
...
struct IPropertyValueWrapper : IUnknown {
  ...
  virtual void *GetValue(
      const GUID &propertyTypeId,
      size_t valueSize) noexcept = 0;
};
...
struct PropertyValueWrapper : Mso::UnknownObject<IPropertyValueWrapper> {
  ...
  PropertyValueWrapper(TProperty &&value) noexcept
  ...
  void *GetValue(
    ...
    VerifyElseCrashSzTag(
        propertyTypeId == PropertyTypeGuid<TProperty>::Value,
        "Incorrect value type",
        0x0281d359 /* tag_c63nz */);
    VerifyElseCrashSzTag(
        valueSize == sizeof(TProperty),
        "Incorrect value size",
        0x0281d35a /* tag_c63n0 */);
    return &m_value;
  }
...
struct PropertyTraits {
  // Use Mso::optional for return type to indicate missing values.
  using ReturnType = Mso::optional<TProperty>;

  static Mso::TCntPtr<IUnknown> ToIUnknown(const TProperty &value) noexcept {
    return Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(value);
  }

  static Mso::TCntPtr<IUnknown> ToIUnknown(TProperty &&value) noexcept {
    return Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(
        std::move(value));
  }

  static Mso::TCntPtr<IUnknown> ToIUnknown(
      const Mso::optional<TProperty> &value) noexcept {
    return value ? Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(*value)
                 : nullptr;
  }

  static Mso::TCntPtr<IUnknown> ToIUnknown(
      Mso::optional<TProperty> &&value) noexcept {
    return value ? Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(
                       std::move(*value))
                 : nullptr;
  }

  static Mso::optional<TProperty> FromIUnknown(
      Mso::TCntPtr<IUnknown> &&obj) noexcept {
    if (obj) {
      auto &wrapper = query_cast<IPropertyValueWrapper &>(*obj);
      void *valuePtr = wrapper.GetValue(
          PropertyTypeGuid<TProperty>::Value, sizeof(TProperty));
      return Mso::optional<TProperty>{*static_cast<TProperty *>(valuePtr)};
    }
    return {};
  }
};
...
struct PropertyTraits<
    TProperty,
    std::enable_if_t<std::is_base_of<IUnknown, TProperty>::value>> {
  using ReturnType = Mso::TCntPtr<TProperty>;

  template <class TValue>
  static Mso::TCntPtr<IUnknown> ToIUnknown(TValue &&value) noexcept {
    return Mso::TCntPtr<IUnknown>(std::forward<TValue>(value));
  }

  static Mso::TCntPtr<TProperty> FromIUnknown(
      Mso::TCntPtr<IUnknown> &&obj) noexcept {
    Mso::TCntPtr<TProperty> result;
    if (obj) {
...
//! Property traits for property types inherited from IUnknown.
//! The type must be used as an Mso::TCntPtr pointer.
template <class TProperty>
struct PropertyTraits<
    Mso::TCntPtr<TProperty>,
    std::enable_if_t<std::is_base_of<IUnknown, TProperty>::value>> {
  using ReturnType = Mso::TCntPtr<TProperty>;
...
//! The isolationKey allows to have properties with the same name in the
//! different scopes. It also allows to define properties local to some
//! component that does not share its isolationKey. The isolationKey with a null
//! value enables access to unscoped properties.
...
struct DECLSPEC_NOVTABLE IPropertyBag : IUnknown {
  ...
  virtual Mso::TCntPtr<IUnknown> Get(
      const void *isolationKey,
      const char *propertyName) const noexcept = 0;

  //! Ensures that property with the provided isolationKey and propertyName
  //! exists by calling valueMaker if needed, and then returns the property
  //! value. The function may return null if valueMaker returns null when
  //! called.
  virtual Mso::TCntPtr<IUnknown> GetOrCreate(
      const void *isolationKey,
      const char *propertyName,
      const Mso::FunctorRef<Mso::TCntPtr<IUnknown>()> &valueMaker) noexcept = 0;
  ...
  virtual Mso::TCntPtr<IUnknown> Replace(
      const void *isolationKey,
      const char *propertyName,
      Mso::TCntPtr<IUnknown> &&value) noexcept = 0;
};

//! Encapsulates property name and type in the same variable.
//! The typical use is to define properties as constants, and then use them in
//! PropertyBag API.
template <class T>
...
//! Encapsulates isolation key, property name, and property type in the same
//! variable. The typical use is to define properties as constants, and then use
//! them in PropertyBag API.
template <class T>
...
//! PropertyBag implementation that can store strongly-typed properties in a
//! thread-safe way. Types inherited from IUnknown are store directly as
//! Mso::TCntPtr<T>. Values of other types are wrapped up into
//! PropertyValueWrapper object.
struct PropertyBag {
  //! Property return type is either Mso::TCntPtr<TProperty> or
  //! Mso::optional<TProperty>. Mso::TCntPtr<TProperty> is returned for types
  //! inherited from IUnknown. The Mso::optional<TProperty> is returned for all
  //! other types.
  template <class TProperty>
  ...
  LIBLET_PUBLICAPI PropertyBag(Mso::TCntPtr<IPropertyBag> &&content) noexcept;

  //! Creates a new instance of PropertyBag with the other PropertyBag content
  //! to share.
  LIBLET_PUBLICAPI PropertyBag(const PropertyBag &other) noexcept;

  //! Creates a new instance of PropertyBag with content taken from the other
  //! PropertyBag. The other PropertyBag content becomes null.
  ...
  //! Assigns the content taken from the other PropertyBag. The other
  //! PropertyBag content becomes null.
  ...
  template <class TProperty>
  ReturnType<TProperty> Get(const NamedProperty<TProperty> &property) const
      noexcept;

  //! Gets property value by isolated named property.
  template <class TProperty>
  ReturnType<TProperty> Get(
      const IsolatedNamedProperty<TProperty> &property) const noexcept;

  //! Gets property value by isolationKey and property name.
  template <class TProperty>
  ReturnType<TProperty> Get(const void *isolationKey, const char *propertyName)
      const noexcept;
  ...
  ReturnType<TProperty> Get(
      const void *isolationKey,
      const NamedProperty<TProperty> &property) const noexcept;

  //! Ensures that property is created by calling valueMaker if needed, and
  //! returns value by property name.
  template <class TProperty>
  ReturnType<TProperty> GetOrCreate(
      const char *propertyName,
      const Mso::FunctorRef<ReturnType<TProperty>()> &valueMaker) const
      noexcept;

  //! Ensures that property is created by calling valueMaker if needed, and
  //! returns value by named property.
  template <class TProperty>
  ReturnType<TProperty> GetOrCreate(
  ...
  //! Ensures that property is created by calling valueMaker if needed, and
  //! returns value by isolated named property.
  template <class TProperty>
  ReturnType<TProperty> GetOrCreate(
  ...
  //! Ensures that property is created by calling valueMaker if needed, and
  //! returns value by isolationKey and property name.
  template <class TProperty>
  ReturnType<TProperty> GetOrCreate(
      const void *isolationKey,
      const char *propertyName,
  ...
  //! Ensures that property is created by calling valueMaker if needed, and
  //! returns value by isolationKey and named property.
  template <class TProperty>
  ReturnType<TProperty> GetOrCreate(
      const void *isolationKey,
      const NamedProperty<TProperty> &property,
  ...
  LIBLET_PUBLICAPI void Remove(
      const void *isolationKey,
      const char *propertyName) const noexcept;
  ...
  template <class TProperty, class TValue>
  void Set(const NamedProperty<TProperty> &property, TValue &&value) const
      noexcept;

  //! Sets property value by isolated named property.
  template <class TProperty, class TValue>
  void Set(const IsolatedNamedProperty<TProperty> &property, TValue &&value)
      const noexcept;

  //! Sets property value by isolationKey, property type and name.
  template <class TProperty, class TValue>
  void Set(const void *isolationKey, const char *propertyName, TValue &&value)
      const noexcept;
 ...
 private:
  Mso::TCntPtr<IPropertyBag> m_content;
};
...
inline bool operator==(
    const PropertyBag &left,
    const PropertyBag &right) noexcept;
...
template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::Get(
    const char *propertyName) const noexcept {
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->Get(nullptr, propertyName));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::Get(
    const NamedProperty<TProperty> &property) const noexcept {
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->Get(nullptr, property.Name));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::Get(
    const IsolatedNamedProperty<TProperty> &property) const noexcept {
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->Get(property.IsolationKey, property.Name));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::Get(
    ...
    const char *propertyName) const noexcept {
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->Get(isolationKey, propertyName));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::Get(
    ...
    const NamedProperty<TProperty> &property) const noexcept {
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->Get(isolationKey, property.Name));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(
  ...
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->GetOrCreate(
          property.IsolationKey, property.Name, [&valueMaker]() noexcept {
            return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
          }));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(
  ...
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->GetOrCreate(
          isolationKey, propertyName, [&valueMaker]() noexcept {
            return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
          }));
}
...
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(
  ...
  return Details::PropertyTraits<TProperty>::FromIUnknown(
      m_content->GetOrCreate(
          isolationKey, property.Name, [&valueMaker]() noexcept {
            return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
          }));
}

template <class TProperty>
void PropertyBag::Remove(const NamedProperty<TProperty> &property) const
    noexcept {
  m_content->Replace(nullptr, property.Name, nullptr);
}

template <class TProperty>
void PropertyBag::Remove(const IsolatedNamedProperty<TProperty> &property) const
    noexcept {
  m_content->Replace(property.IsolationKey, property.Name, nullptr);
}

template <class TProperty>
void PropertyBag::Remove(
    const void *isolationKey,
    const NamedProperty<TProperty> &property) const noexcept {
...
void PropertyBag::Set(const char *propertyName, TValue &&value) const noexcept {
  m_content->Replace(
      nullptr,
      propertyName,
      Details::PropertyTraits<TProperty>::ToIUnknown(
          std::forward<TValue>(value)));
}

template <class TProperty, class TValue>
void PropertyBag::Set(const NamedProperty<TProperty> &property, TValue &&value)
    const noexcept {
  m_content->Replace(
      nullptr,
      property.Name,
