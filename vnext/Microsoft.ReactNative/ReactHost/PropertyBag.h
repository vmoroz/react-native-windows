...
//!   association in this header file. Values of other types are wrapped up into Mso::optional.
//!
...
{
	PropertyValueWrapper(const TProperty& value) noexcept
		: m_value{value}
	{}
	...
	{
		VerifyElseCrashSzTag(propertyTypeId == PropertyTypeGuid<TProperty>::Value, "Incorrect value type", 0x0281d359 /* tag_c63nz */);
...
struct PropertyTraits
{
	...
	static Mso::TCntPtr<IUnknown> ToIUnknown(const Mso::optional<TProperty>& value) noexcept
	{
		return value ? Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(*value) : nullptr;
	}

	static Mso::TCntPtr<IUnknown> ToIUnknown(Mso::optional<TProperty>&& value) noexcept
	{
		return value ? Mso::Make<PropertyValueWrapper<TProperty>, IUnknown>(std::move(*value)) : nullptr;
	}

	static Mso::optional<TProperty> FromIUnknown(Mso::TCntPtr<IUnknown>&& obj) noexcept
	{
		if (obj)
		{
...
struct PropertyTraits<TProperty, std::enable_if_t<std::is_base_of<IUnknown, TProperty>::value>>
{
	...
	static Mso::TCntPtr<TProperty> FromIUnknown(Mso::TCntPtr<IUnknown>&& obj) noexcept
	{
...
template <class TProperty>
struct PropertyTraits<Mso::TCntPtr<TProperty>, std::enable_if_t<std::is_base_of<IUnknown, TProperty>::value>>
{
	using ReturnType = Mso::TCntPtr<TProperty>;
...
struct DECLSPEC_NOVTABLE IPropertyBag : IUnknown
{
	...
	virtual Mso::TCntPtr<IUnknown> Get(const void* isolationKey, const char* propertyName) const noexcept = 0;
	...
	virtual Mso::TCntPtr<IUnknown> GetOrCreate(const void* isolationKey, const char* propertyName,
	...
	virtual Mso::TCntPtr<IUnknown> Replace(const void* isolationKey, const char* propertyName,
...
//! Types inherited from IUnknown are store directly as Mso::TCntPtr<T>.
//! Values of other types are wrapped up into PropertyValueWrapper object.
struct PropertyBag
{
	//! Property return type is either Mso::TCntPtr<TProperty> or Mso::optional<TProperty>.
	//! Mso::TCntPtr<TProperty> is returned for types inherited from IUnknown.
	//! The Mso::optional<TProperty> is returned for all other types.
	template <class TProperty>
	...
	template <class TProperty>
	ReturnType<TProperty> GetOrCreate(const char* propertyName,
	...
	template <class TProperty>
	ReturnType<TProperty> GetOrCreate(const NamedProperty<TProperty>& property,
		const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept;

	//! Ensures that property is created by calling valueMaker if needed, and returns value by isolated named property.
	template <class TProperty>
	ReturnType<TProperty> GetOrCreate(const IsolatedNamedProperty<TProperty>& property,
		const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept;

	//! Ensures that property is created by calling valueMaker if needed, and returns value by isolationKey and property name.
	template <class TProperty>
	ReturnType<TProperty> GetOrCreate(const void* isolationKey, const char* propertyName,
		const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept;

	//! Ensures that property is created by calling valueMaker if needed, and returns value by isolationKey and named property.
	template <class TProperty>
	ReturnType<TProperty> GetOrCreate(const void* isolationKey, const NamedProperty<TProperty>& property,
		const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept;
...
template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::Get(const void* isolationKey, const NamedProperty<TProperty>& property) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->Get(isolationKey, property.Name));
}

template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(const char* propertyName,
	const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->GetOrCreate(nullptr, propertyName, [&valueMaker]() noexcept {
		return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
	}));
}

template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(const NamedProperty<TProperty>& property,
	const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->GetOrCreate(nullptr, property.Name, [&valueMaker]() noexcept {
		return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
	}));
}

template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(const IsolatedNamedProperty<TProperty>& property,
	const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->GetOrCreate(property.IsolationKey, property.Name, [&valueMaker]() noexcept {
		return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
	}));
}

template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(const void* isolationKey, const char* propertyName,
	const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->GetOrCreate(isolationKey, propertyName, [&valueMaker]() noexcept {
		return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
	}));
}

template <class TProperty>
PropertyBag::ReturnType<TProperty> PropertyBag::GetOrCreate(const void* isolationKey, const NamedProperty<TProperty>& property,
	const Mso::FunctorRef<ReturnType<TProperty>()>& valueMaker) const noexcept
{
	return Details::PropertyTraits<TProperty>::FromIUnknown(m_content->GetOrCreate(isolationKey, property.Name, [&valueMaker]() noexcept {
		return Details::PropertyTraits<TProperty>::ToIUnknown(valueMaker());
	}));
}
...
{
	m_content->Replace(nullptr, propertyName, Details::PropertyTraits<TProperty>::ToIUnknown(std::forward<TValue>(value)));
}

template <class TProperty, class TValue>
void PropertyBag::Set(const NamedProperty<TProperty>& property, TValue&& value) const noexcept
{
	m_content->Replace(nullptr, property.Name, Details::PropertyTraits<TProperty>::ToIUnknown(std::forward<TValue>(value)));
}

template <class TProperty, class TValue>
void PropertyBag::Set(const IsolatedNamedProperty<TProperty>& property, TValue&& value) const noexcept
{
	m_content->Replace(property.IsolationKey, property.Name, Details::PropertyTraits<TProperty>::ToIUnknown(std::forward<TValue>(value)));
}

template <class TProperty, class TValue>
void PropertyBag::Set(const void* isolationKey, const char* propertyName, TValue&& value) const noexcept
{
	m_content->Replace(isolationKey, propertyName, Details::PropertyTraits<TProperty>::ToIUnknown(std::forward<TValue>(value)));
}

template <class TProperty, class TValue>
void PropertyBag::Set(const void* isolationKey, const NamedProperty<TProperty>& property, TValue&& value) const noexcept
{
	m_content->Replace(isolationKey, property.Name, Details::PropertyTraits<TProperty>::ToIUnknown(std::forward<TValue>(value)));
}
