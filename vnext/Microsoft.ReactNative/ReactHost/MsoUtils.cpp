...
public:
	WhenAllCompletedState(size_t taskCount) noexcept
		: m_taskResults{taskCount, s_completionGuardErrorProvider.MakeMaybe<void>()}
		, m_waitCount{taskCount}
	{
	...
	{
		VerifyElseCrashSzTag(index < m_taskResults.size(), "Index must be less than m_taskResults.size()", 0x0281e3d4 /* tag_c64pu */);
		auto& resultSlot = m_taskResults[index];
		VerifyElseCrashSzTag(resultSlot.IsError() && resultSlot.GetError().As(s_completionGuardErrorProvider),
			"The result slot must be unassigned", 0x0281e3d5 /* tag_c64pv */);
