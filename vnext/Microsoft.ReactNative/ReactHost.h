	...
	: Mso::RefCountedObjectNoVTable<Mso::RefCountStrategy::WeakRef, AsyncActionQueue>
{
	...
	AsyncActionQueue(Mso::Async::IDispatchQueue& queue) noexcept;
	...
	Mso::Async::IDispatchQueue& Queue() noexcept;
...
class ReactHost final
	: public Mso::ActiveObject<IReactHost>
{
...
public: // IReactHost
	...
	Mso::TCntPtr<IReactViewHost> MakeViewHost(ReactViewOptions&& options) noexcept override;
	Mso::Future<std::vector<Mso::TCntPtr<IReactViewHost>>> GetViewHostList() noexcept override;
...
private:
	...
	static Mso::TCntPtr<Mso::Async::IDispatchQueue> GetNativeQueue(const ReactOptions& options) noexcept;
...
private:
	...
	const Mso::ActiveReadableField<Mso::TCntPtr<AsyncActionQueue>> m_actionQueue { Mso::Make<AsyncActionQueue>(Queue()), Queue(), m_mutex };
	...
	const Mso::ActiveReadableField<Mso::TCntPtr<IReactInstanceInternal>> m_reactInstance { Queue(), m_mutex };
...
private:
	...
	const Mso::ActiveField<Mso::TCntPtr<IReactViewInstance>> m_viewInstance { Queue() };
	const Mso::ActiveField<size_t> m_pendingUnloadActionId { 0, Queue() };
	const Mso::ActiveField<size_t> m_nextUnloadActionId { 0, Queue() };
