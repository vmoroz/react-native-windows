...
constexpr const Mso::JSHost::NamedProperty<Mso::Async::IDispatchQueue> JSDispatchQueueProperty { "JSDispatchQueue" };

//! PropertyBag named property for Native dispatch queue.
constexpr const Mso::JSHost::NamedProperty<Mso::Async::IDispatchQueue> NativeDispatchQueueProperty { "NativeDispatchQueue" };
...
struct IReactRootView : public IUnknown
{
	...
	virtual Mso::TCntPtr<IReactInstance> GetReactInstance() const noexcept = 0;
...
struct ReactDevOptions
{
	...
	//! Enables debugging using the web debugger. By default, this is Chrome using http://localhost:8081/debugger-ui from Metro/Haul.
...
struct ReactOptions
{
	...
	std::vector<Mso::TCntPtr<IJSBundle>> JSBundles;
...
struct DECLSPEC_NOVTABLE IReactHost : IUnknown
{
	...
	virtual Mso::TCntPtr<IReactInstance> Instance() const noexcept = 0;
	...
	virtual Mso::Async::IDispatchQueue& NativeQueue() const noexcept = 0;
	...
	virtual Mso::TCntPtr<IReactViewHost> MakeViewHost(ReactViewOptions&& options) noexcept = 0;

	//! Returns a snapshot of IReactViewHost instances associated with the IReactHost.
	virtual Mso::Future<std::vector<Mso::TCntPtr<IReactViewHost>>> GetViewHostList() noexcept = 0;
};
...
struct IReactViewInstance : IUnknown
{
	...
	virtual Mso::Future<void> Reload(Mso::TCntPtr<IReactInstance>&& reactInstance, ReactViewOptions&& viewOptions) noexcept = 0;
...
LIBLET_PUBLICAPI Mso::TCntPtr<IReactHost> MakeReactHost(ReactOptions&& options) noexcept;
...
LIBLET_PUBLICAPI Mso::TCntPtr<IReactHost> MakeReactHost(
...
LIBLET_PUBLICAPI Mso::TCntPtr<IReactInstance> GetOrCreateReactInstance(const std::string& sdxIdentity,
	Mso::JSHost::OnInitializedCallback&& onInitialized = nullptr) noexcept;

//! Test hook to cleanup the instance vector.
//! without this motif will complain of memory leak.
LIBLET_PUBLICAPI void TestOnly_CleanupInstanceList() noexcept;
...
inline Mso::TCntPtr<IReactNativeHost> MakeReactNativeHost(ReactOptions&& options) noexcept
{
