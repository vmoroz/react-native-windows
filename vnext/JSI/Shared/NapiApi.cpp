...
NapiApi::NapiRefHolder::NapiRefHolder(napi_env env, napi_value value) noexcept : m_env{env} {
  CHECK_NAPI(napi_create_reference(m_env, value, 1, &m_ref));
}
...
NapiApi::NapiRefHolder::~NapiRefHolder() noexcept {
  if (m_ref) {
    ...
    CHECK_NAPI(napi_delete_reference(m_env, std::exchange(m_ref, nullptr)));
  }
}
...
[[noreturn]] void NapiApi::ThrowJsException(JsErrorCode errorCode) {
...
/*static*/ JsValueRef NapiApi::GetUndefinedValue() {
  JsValueRef undefinedValue{JS_INVALID_REFERENCE};
  NapiVerifyJsErrorElseThrow(JsGetUndefinedValue(&undefinedValue));
  return undefinedValue;
}

/*static*/ JsValueRef NapiApi::GetNullValue() {
  JsValueRef nullValue{JS_INVALID_REFERENCE};
  NapiVerifyJsErrorElseThrow(JsGetNullValue(&nullValue));
  return nullValue;
}

/*static*/ JsValueRef NapiApi::BoolToBoolean(bool value) {
  JsValueRef booleanValue{JS_INVALID_REFERENCE};
  NapiVerifyJsErrorElseThrow(JsBoolToBoolean(value, &booleanValue));
  return booleanValue;
}

/*static*/ bool NapiApi::BooleanToBool(JsValueRef value) {
  bool boolValue{false};
  NapiVerifyJsErrorElseThrow(JsBooleanToBool(value, &boolValue));
  return boolValue;
}

/*static*/ JsValueType NapiApi::GetValueType(JsValueRef value) {
  JsValueType type{JsValueType::JsUndefined};
  NapiVerifyJsErrorElseThrow(JsGetValueType(value, &type));
  return type;
}
