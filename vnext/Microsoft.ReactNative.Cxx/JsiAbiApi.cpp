...
JsiBufferWrapper::JsiBufferWrapper(std::shared_ptr<Buffer const> const &buffer) noexcept
    : m_buffer{buffer} {}
...
void JsiHostObjectWrapper::SetProperty(IJsiRuntime const &runtime, JsiPointerHandle name, JsiValueData const &value) {
  ...
  m_hostObject->set(
      rt,
      *reinterpret_cast<PropNameID *>(name),
      *reinterpret_cast<Value const *>(&value));
}
...
/*static*/ std::shared_ptr<HostObject> JsiHostObjectWrapper::GetHostObject(
...
/*static*/ HostFunctionType &JsiHostFunctionWrapper::GetHostFunction(
...
JsiAbiRuntime::~JsiAbiRuntime() = default;
  ...
  /*static*/ JsiPointerHandle JsiAbiRuntime::ToJsiPointerHandle(
      Runtime::PointerValue const *pointerValue) noexcept {
  ...
  Value JsiAbiRuntime::evaluateJavaScript(
  ...
  Value JsiAbiRuntime::evaluatePreparedJavaScript(
  ...
  Value JsiAbiRuntime::getProperty(
      const Object &obj,
  ...
  Value JsiAbiRuntime::callAsConstructor(
      const Function &func,
      const Value *args,
      size_t count) {
    JsiValueData const *argsData = reinterpret_cast<JsiValueData const *>(args);
