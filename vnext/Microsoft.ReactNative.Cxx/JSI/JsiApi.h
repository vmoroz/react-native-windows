...
// If it is not found, then create it and store in the context.Properties().
...
facebook::jsi::Runtime &GetOrCreateContextRuntime(ReactContext const &context) noexcept {
  ...
  if (!runtime) {
    ...
    ReactPropertyId<ReactNonAbiValue<std::unique_ptr<JsiAbiRuntime>>> jsiRuntimeProperty{
        L"ReactNative.InstanceData", jsiRuntimeLocalName.c_str()};
    ReactNonAbiValue<std::unique_ptr<JsiAbiRuntime>> runtimeValue{std::in_place, std::move(runtimeHolder)};
    context.Properties().Set(jsiRuntimeProperty, runtimeValue);

    // We remove the JSi runtime from properties when React instance is destoryed.
